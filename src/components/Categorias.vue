            ...
            <template v-for="(i, index) in categorias">
              <v-flex :key="i._id" v-if="i.tipo ==2" xs6 md4 lg3>
                ...
                <v-card-actions class="white justify-center">
                  <div v-if="isLogged">
                    <v-btn class="primary ma-1"  v-if="!i.seguida" v-on:click="seguir(i._id,index)" :id="i._id">
                      <v-icon >mdi-plus-circle-outline</v-icon>Seguir
                    </v-btn>
                    <v-btn class="error ma-1"  v-if="i.seguida" v-on:click="dejar(i._id,index)" :id="i._id">
...
<script>
import { getAllCategorias, getAllCategoriasLogin } from "../services/sCategoria"
...
export default {
  ...
  methods:{
    seguir: function (e,index){
        agregarFollowing(e, this.$store.state.usuario.token).then( res => {
          this.categorias[index].seguida = true;
  ...
  created: function() {
    if(this.isLogged) {
      console.log("entro")
      getAllCategoriasLogin(this.$store.state.usuario.id).then( res => {
