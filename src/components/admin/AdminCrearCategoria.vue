<template>
	<v-container>
		<v-row col-12>
			<v-card  class="mx-3" width="100%">
				...
				<v-card-actions style="margin-top:10px">
					<v-spacer></v-spacer>
					<v-btn @click="postCategoria()" color="success">Crear nueva categoria</v-btn>
				</v-card-actions>
			</v-card>
		</v-row>
		<!-- Notificaciones -->
    	...
    	>
			<v-card>
				<v-card-title class="headline">¡Categoria creada!</v-card-title>
				<v-card-text>
				<v-layout align-center justify-center>
					<p><v-icon size="100" color="green">mdi-checkbox-marked-circle-outline</v-icon></p>
				</v-layout>
				La categoria ha sido añadida a la base de datos.
				</v-card-text>
				<v-card-actions>
				<v-spacer></v-spacer>
...
<script>
import { crearCategoria, eliminarCategoria } from "./../../services/sCategoria";
...
export default {
    ...
    methods: {
        ...
        postCategoria: function() {
			if( this.imagen.length === 0 || this.nombre === '' || this.tipo === "") {
				this.dialog.message =
            ...
            subirImagenSingle(this.imagen, this.$store.state.usuario.token).then( res => {
                crearCategoria(this.nombre, this.tipo, res.data.file_path, this.$store.state.usuario.token).then( res => {
