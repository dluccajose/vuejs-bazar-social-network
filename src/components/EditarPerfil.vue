<template>
	<v-app id="inspire">
		<v-content>
			<v-container fluid fill-height>
				<v-layout align-center justify-center>
							...
							<v-toolbar color="primary" dark flat>
								...
								<v-tooltip right>
									<template v-slot:activator="{ on }">
										<v-btn icon large href="./#/perfil" v-on="on">
					...
					<router-link to="/editarFotoPerfil">
                 		<v-avatar size="120">
                  ...
                  <v-expansion-panel>
                    ...
                    <v-expansion-panel-content>
                        ...
                        disabled
									...
									<v-card-actions>
										<v-flex xs3 sm2 class="mr-3">
											<v-select v-model="usuario.cedulaPrefijo" :items="items" label="V-E " outlined   disabled></v-select>
										</v-flex>
										<v-spacer></v-spacer>
										<v-flex xs9 sm10>
											<v-text-field
                        ...
                        disabled
									...
									<v-flex xs12 sm12>
										 <v-select v-model="usuario.id_estado" item-text="nombre" item-value="_id" :items="localidades" label="Localidad" outlined ></v-select>
									</v-flex>
									<v-flex xs12 sm12>
										<v-text-field
         ...
         <v-expansion-panels>
            <v-expansion-panel class="mt-3 mb-5">
                    <v-expansion-panel-header>Establecer Contraseña</v-expansion-panel-header>
                <v-expansion-panel-content>
									<v-flex xs12 sm12>
										<v-text-field
											v-model="password"
							...
							<v-card-actions>
								...
								<v-spacer></v-spacer>
								<v-spacer></v-spacer>
								<v-btn block color="primary" @click="validar()" dark>Guardar Cambios</v-btn>
								<v-spacer></v-spacer>
							</v-card-actions>
							<v-card-actions>
								<v-spacer></v-spacer>
    	...
    	>
			<v-card>
				<v-card-title class="headline">¡Usuario Actualizado con exito!</v-card-title>
				<v-card-text>
				<v-layout align-center justify-center>
		...
		<!-- Loader -->
		<div class="loader" v-show="isLoading" justify-center >
      		<v-progress-circular
				indeterminate 
				color="purple"
				size="60"
...
import { getUsuario, modificarUsuario } from "../services/sUsuario"
	...
	methods: {
		...
		validar() {
			if(this.password!=this.confirmPassword){
				this.dialog.message = "La contraseña no coincide";
	  ...
	  this.usuario.password=this.password
			modificarUsuario(this.usuario,this.$store.state.usuario.token)
				.then(res => {
					console.log(res);
